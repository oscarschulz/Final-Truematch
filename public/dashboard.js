        ...
        row.addEventListener('click', async () => {
          ...
          try {
            ...
            if (path.endsWith('/dashboard.html') || path === '/dashboard.html' || path.endsWith('dashboard.html')) {
              ...
              const focus = String(u.searchParams.get('focus') || u.searchParams.get('match') || '').trim().toLowerCase();
...
function updateMatchPreview(peerEmail, lastText, { bumpToTop = true } = {}) {
  ...
  // Keep latest message stored for click/search/inbox behavior,
  // but render presence-based status on the visible card line.
...
function getMatchCardPresenceText(lastSeenAtMs) {
  const ms = Number(lastSeenAtMs || 0);
  if (!ms) return 'Recently active';
  const isOnline = (Date.now() - ms) <= PRESENCE_ONLINE_WINDOW_MS;
...
function renderMatchCardPresence(card) {
  ...
  const lastEl = card.querySelector('.match-last');
  if (!lastEl) return;
  const lastSeenAtMs = Number(card.dataset.lastSeenAtMs || card.getAttribute('data-last-seen-at-ms') || 0);
  lastEl.textContent = getMatchCardPresenceText(lastSeenAtMs);
}

function refreshAllMatchCardPresence() {
  if (!DOM.matchesContainer) return;
  const cards = Array.from(DOM.matchesContainer.querySelectorAll('.match-card'));
  cards.forEach(renderMatchCardPresence);
}
      ...
      return `
        <div class="match-card" data-name="${safeName}" data-email="${m.email || ''}" data-photo-url="${photoUrl}" data-last-seen-at-ms="${m.lastSeenAtMs || 0}" data-msg="${msg}" data-unread="${hasUnread ? '1' : '0'}">
          ...
          <div class="match-info">
            ...
            <div class="match-last" style="${lastStyle}">${getMatchCardPresenceText(m.lastSeenAtMs)}</div>
          </div>
        </div>
      `;
    }).join('');
