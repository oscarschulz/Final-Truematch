    ...
    preConfirm: async () => {
      ...
      try {
        ...
        if (code === 'wrong_password') {
          ...
          Swal.showValidationMessage('Your new password is too weak.');
        } else if (code === 'too_many_requests') {
          Swal.showValidationMessage('Too many attempts. Try again later.');
        } else if (code === 'auth_backend_misconfigured') {
          Swal.showValidationMessage('Password updates are temporarily unavailable.');
        } else if (code === 'user_not_found' || code === 'not_logged_in') {
          Swal.showValidationMessage('Please sign in again and retry.');
        } else {
          Swal.showValidationMessage(code || 'Failed to update password.');
        }
        return false;
      } catch (err) {
        Swal.showValidationMessage(String(err?.message || 'Failed to update password.'));
        return false;
      }
    },
  }).then((result) => {
